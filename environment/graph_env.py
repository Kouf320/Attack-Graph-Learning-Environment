...
import re
import logging
from rewards.attacker_reward import RewardModel, DefaultRewardModel
...
class GraphEnvironment:
    ...
    def __init__(self, graph_json, goal_node=None, config_path='config.json'):
        ...
        self.NORM_F = env_config['NORM_F']
        self.NORM_F_ATTEMPT = env_config['NORM_F_ATTEMPT']
        self.NON_ACTION_VALUE = env_config['NON_ACTION_VALUE']
        ...
        self.cumulative_alert_count = np.zeros(self.num_nodes, dtype=np.float32)
        self.last_alert_step = -np.ones(self.num_nodes, dtype=np.float32)
        self.defender_action_flags = np.zeros((self.num_nodes, 3), dtype=np.float32)
        self._goal_distances = None
        self._max_cvss_incoming = None

        self.nodes = list(self.graph.nodes)
        self.num_nodes = len(self.nodes)

        self.end_node = self._resolve_goal_node(goal_node)
        ...
        if not self.candidates:
            self.candidates = [n for n in self.nodes if n != self.end_node]
    ...
    def _resolve_goal_node(self, goal_node):
        if goal_node is not None:
            node_str = str(goal_node)
            if node_str not in self.graph.nodes:
                raise ValueError(
                    f"goal_node '{goal_node}' not found in graph. "
                    f"Available node IDs (first 10): {list(self.graph.nodes)[:10]}"
                )
            return node_str
        access_nodes = [
            k for k, v in self.json_graph.get('assets', {}).items()
            if v.get('metaconcept') == 'Access'
        ]
        if access_nodes:
            return str(max(access_nodes, key=lambda x: int(x) if x.isdigit() else 0))
        return str(max(self.nodes, key=lambda x: int(x) if x.isdigit() else 0))
    ...
    def reset_recon(self, episode):
        ...
        for node_id in self.graph.nodes:
            self.graph.nodes[node_id]['cum_src_count'] = 0
            self.graph.nodes[node_id]['cum_dst_count'] = 0
            self.graph.nodes[node_id]['alert_volume_src'] = 0.0
            self.graph.nodes[node_id]['alert_volume_dst'] = 0.0
            self.graph.nodes[node_id]['z_score'] = 0.0
            self.graph.nodes[node_id]['entropy'] = 0.0
        ...
        for ip in alerted_ips:
            if ip in self.ip_to_nodes_map:
                for node_id in self.ip_to_nodes_map[ip]:
                    ...
                    if idx is not None:
                        self.cumulative_alert_count[idx] += 1.0
                        self.last_alert_step[idx] = float(self._current_step)
        self._goal_distances = None
    ...
    def get_valid_actions(self, node):
        """Returns only successors reachable through unmasked edges and unmasked nodes."""
        valid = []
        u_idx = self.node_to_idx.get(node)
        if u_idx is None or self.node_mask[u_idx] == 0:
            return valid

        for successor in self.graph.successors(node):
            v_idx = self.node_to_idx.get(successor)
            if v_idx is not None and self.node_mask[v_idx] == 1:
    ...
    def apply_action(self, action_id, target_node=None, **kwargs):
        ...
        if action_id == 0:
            ...
            result = self._action_network_filtering(source_node=target_node, **kwargs)
            if result.get('status') == 'success':
                self.update_defender_action_flags(target_node, 1)
                self._goal_distances = None
            return result

        elif action_id == 2:
            result = self._action_restore_software(target_node_id=target_node, **kwargs)
            if result.get('status') == 'success':
                self.update_defender_action_flags(target_node, 2)
            return result

        elif action_id == 3:
            result = self._action_restore_network_connection(target_node_id=target_node, **kwargs)
            if result.get('status') == 'success':
                self.update_defender_action_flags(target_node, 3)
                self._goal_distances = None
            return result
    ...
    def _action_network_filtering(self, source_node, debug=False):
        """
        Blocks outgoing traffic FROM the source_node (target selected by agent).
        """
    ...
    def _action_restore_software(self, target_node_id=None, ip_address=None, cve_id=None, debug=False):
        """
        Action 2: Restore Software (Smart Patching).
        Masks (Disables) the Vulnerability Node in the Attack Graph.
        
        Logic:
        - If target is a CVE Node: Mask it directly.
        - If target is a Host Node: Find attached CVE neighbors and mask them.
        """
        nodes_to_mask = []
    ...
    def _action_restore_network_connection(self, target_node_id=None, debug=False):
        """
        Restores a blocked edge specifically involving the target_node.
        """
    ...
    def _action_isolate_ip(self, ip_address, debug=False):
        """
        Isolates ALL nodes containing the given IP string (e.g. '192.168.1.50').
        """
    ...
    def _compute_goal_distances(self):
        """Compute shortest-path hop distance from every node to the goal on the current masked graph."""
        masked_edges = []
        for u, v in self.graph.edges():
            u_idx = self.node_to_idx.get(u)
            v_idx = self.node_to_idx.get(v)
            if u_idx is not None and v_idx is not None:
                if self.edge_mask[u_idx, v_idx] == 1 and self.node_mask[u_idx] == 1 and self.node_mask[v_idx] == 1:
                    masked_edges.append((u, v))

        subgraph = nx.DiGraph()
        subgraph.add_edges_from(masked_edges)

        distances = np.full(self.num_nodes, -1.0, dtype=np.float32)
        try:
            path_lengths = nx.single_target_shortest_path_length(subgraph, self.end_node)
            if isinstance(path_lengths, dict):
                path_lengths = path_lengths.items()
            for node, hops in path_lengths:
                idx = self.node_to_idx.get(node)
                if idx is not None:
                    distances[idx] = float(hops)
        except (nx.NetworkXError, nx.NodeNotFound):
            pass

        max_dist = distances[distances >= 0].max() if (distances >= 0).any() else 1.0
        if max_dist == 0:
            max_dist = 1.0
        self._goal_distances = distances
        self._goal_max_dist = max_dist

    def _compute_max_cvss_incoming(self):
        """Compute max CVSS (edge distance) on incoming edges for each node. Static per graph."""
        max_cvss = np.zeros(self.num_nodes, dtype=np.float32)
        all_dists = []
        for u, v, data in self.graph.edges(data=True):
            d = data.get('distance', 0.0)
            if d > 0:
                all_dists.append(d)
        global_max = max(all_dists) if all_dists else 1.0
        ...
        for node in self.nodes:
            ...
            if predecessors:
                ...
                for pred in predecessors:
                    ...
                    incoming_dists.append(edge_data.get('distance', 0.0))
                max_cvss[idx] = max(incoming_dists) / global_max
            else:
                max_cvss[idx] = 0.0

        self._max_cvss_incoming = max_cvss

    def update_defender_action_flags(self, target_node_id, action_type):
        """Called after apply_action to update per-node defender history flags.
        action_type: 1=Network Filtering, 2=Restore Software, 3=Restore Connection"""
        idx = self.node_to_idx.get(target_node_id)
        if idx is not None and action_type in (1, 2, 3):
            self.defender_action_flags[idx, action_type - 1] = 1.0
    ...
    def get_graph_observation(self):
        ...
        if self._goal_distances is None:
            self._compute_goal_distances()
        if self._max_cvss_incoming is None:
            self._compute_max_cvss_incoming()

        current_step = max(self._current_step, 1)
        ...
        for node in self.nodes:
            ...
            f_cum_alerts = self.cumulative_alert_count[idx] / current_step

            if self.last_alert_step[idx] >= 0:
                steps_since = current_step - self.last_alert_step[idx]
                f_alert_recency = 1.0 / (steps_since + 1.0)
            else:
                f_alert_recency = 0.0

            raw_dist = self._goal_distances[idx]
            f_goal_dist = (raw_dist / self._goal_max_dist) if raw_dist >= 0 else 0.0

            f_max_cvss = float(self._max_cvss_incoming[idx])

            f_def_filtered = float(self.defender_action_flags[idx, 0])
            f_def_patched = float(self.defender_action_flags[idx, 1])
            f_def_restored = float(self.defender_action_flags[idx, 2])

            node_features.append([
                f_active, f_risk, f_central, f_vuln, f_crit,
                f_zscore, f_entropy, f_is_alerted, f_src_vlm, f_dst_vlm,
                f_cum_alerts, f_alert_recency, f_goal_dist, f_max_cvss,
                f_def_filtered, f_def_patched, f_def_restored,
            ])
    ...
    def get_valid_action_mask(self):
        ...
        for alert_str in self.current_alert_group:
            ...
            for ip in found_ips:
                if ip in self.ip_to_nodes_map and self.ip_to_nodes_map[ip]:
                    alerted_ips.add(ip)

...
class DefaultRewardModel(RewardModel):
    def __init__(self, norm_factor_attempt=21.147181, norm_factor_cia=17.8572, non_action_value=0.0):
        self.NORM_F_ATTEMPT = norm_factor_attempt
        self.NORM_F_CIA = norm_factor_cia
        self.NON_ACTION_VALUE = non_action_value
    ...
    def get_reward(self, **kwargs):
        ...
        if node_position == 0:
            val = attack_metric.get("AV", 0) * attack_metric.get("AC", 0) * attack_metric.get("PR", 0) * attack_metric.get("UI", 0)
            return round(val * self.NORM_F_ATTEMPT, 3)
        
        elif node_position == 1:
            val = (1 - attack_metric.get('C', 0)) * (1 - attack_metric.get('I', 0)) * (1 - attack_metric.get('A', 0))
            return round(val * self.NORM_F_CIA, 3)
    ...
    def _parse_cvss(self, cvss_string):
        ...
        for pair in metric_pairs:
            try:
                ...
                if metric == 'AV': parsed_qn_metrics['AV'] = vector.get(value)
                elif metric == "AC": parsed_qn_metrics['AC'] = complexity.get(value)
                elif metric == "PR": parsed_qn_metrics['PR'] = privileges.get(value)
                elif metric == "UI": parsed_qn_metrics['UI'] = interaction.get(value)
                elif metric in ["C", "I", "A"]: parsed_qn_metrics[metric] = cia.get(value)
            except ValueError:
                continue
